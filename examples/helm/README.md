...
**Migration Toolkit RHCL (optional):** Git-sourced Helm app (`connectivityLink.helmApps` id `migration-toolkit-rhcl`, **`enabled: false` by default**) from [maximilianoPizarro/migration-toolkit-rhcl](https://github.com/maximilianoPizarro/migration-toolkit-rhcl) path `helm/migration-toolkit-rhcl` (GUI by [nmushino](https://github.com/nmushino/migration-toolkit-rhcl); chart staged for upstream). Namespace `migration-toolkit`, route `migration-toolkit.<domain>`, images `quay.io/maximilianopizarro/migration-toolkit-rhcl-{backend,frontend}:v0.1.0`. After Mushino merges the chart, change only `repoURL` to `https://github.com/nmushino/migration-toolkit-rhcl`.
