...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster.example.com/api/v1/customers?tipoCliente=PERSONAL&ciudad=Buenos+Aires&page=0&size=5" \
  | python3 -m json.tool
```
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster.example.com/api/v1/customers/1/credit-score" \
  | python3 -m json.tool
```
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster.example.com/api/v1/customers/1/summary" \
  | python3 -m json.tool
```
...
**URL**: [https://neuralbank.apps.cluster.example.com/q/swagger-ui](https://neuralbank.apps.cluster.example.com/q/swagger-ui)
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  https://neuralbank.apps.cluster.example.com/q/openapi
```
