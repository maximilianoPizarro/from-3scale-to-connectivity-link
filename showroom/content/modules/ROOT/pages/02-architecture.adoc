...
image::cluster-layout.svg["3scale source workloads and Connectivity Link target workloads in the same OpenShift cluster plus Developer Hub",opts=inline]
...
image::flow-3scale.svg["Request path — Client; OpenShift Route; APIcast; Backend Service",opts=inline]
...
image::flow-connectivity-link.svg["Request path — Client; OpenShift Route; Istio Gateway; Backend Service with policies",opts=inline]
