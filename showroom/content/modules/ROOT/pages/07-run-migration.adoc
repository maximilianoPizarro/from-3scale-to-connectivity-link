...
parameters:
  - title: Application Details
    properties:
      ...
      owner:          # e.g. user42 (workshop pattern userN)
  ...
  - title: Source & Target Configuration
    properties:
      ...
      targetNamespace:    # e.g. userN-neuralbank-cl
...
metadata:
  ...
  namespace: {user_name}-neuralbank-cl
  annotations:
...
image::scaffolder-steps.svg["Scaffolder pipeline — template; Gitea; catalog; Argo CD; Mailpit",opts=inline]

The Gitea repository path follows `gitea-gitea.{cluster_domain}/ws-{user_name}/...`. The Argo CD Application name follows `{user_name}-<app>-cl-migration` and syncs into `{user_name}-neuralbank-cl` (or your chosen target namespace).
...
image::resource-mapping.svg[Mapping from 3scale concepts to Connectivity Link resources,opts=inline]
