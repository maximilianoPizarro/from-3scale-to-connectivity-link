...
1. Log in to **Developer Hub**: `https://backstage-developer-hub-developer-hub.apps.cluster-zgksw.dynamic.redhatworkshops.io`
...
```
https://openshift-gitops-server-openshift-gitops.apps.cluster-zgksw.dynamic.redhatworkshops.io
```
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster-zgksw.dynamic.redhatworkshops.io/api/v1/customers/1" \
  | python3 -m json.tool
```
