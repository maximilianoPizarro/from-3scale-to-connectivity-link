...
1. Log in to **Developer Hub**: `https://backstage-developer-hub-developer-hub.apps.cluster-cq9fp.dynamic.redhatworkshops.io`
...
```
https://openshift-gitops-server-openshift-gitops.apps.cluster-cq9fp.dynamic.redhatworkshops.io
```
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster-cq9fp.dynamic.redhatworkshops.io/api/v1/customers/1" \
  | python3 -m json.tool
```
...
curl -s -o /dev/null -w "HTTP Status: %{http_code}\n" \
  https://nfl-wallet.apps.cluster-cq9fp.dynamic.redhatworkshops.io/api/v1/customers
```
