...
- **`provider.issuerURL`**: apunta al realm de Keycloak (`https://rhbk.apps.cluster-2fjv7.dynamic.redhatworkshops.io/realms/neuralbank`)
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster-2fjv7.dynamic.redhatworkshops.io/api/v1/customers/1" \
  | python3 -m json.tool
```
