...
- **`provider.issuerURL`**: apunta al realm de Keycloak (`https://rhbk.apps.cluster.example.com/realms/neuralbank`)
...
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://neuralbank.apps.cluster.example.com/api/v1/customers/1" \
  | python3 -m json.tool
```
